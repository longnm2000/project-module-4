...
export default function OrdersManagerPage() {
    ...
    const loadName = (decoded: DecodedToken | null) => {
        if (decoded) {
    ...
    const handleOrderDetail = async (order: Order) => {
        ...
        await axios
            .get(`http://localhost:3000/api/v1/orders/${order.orderId}`)
    ...
    const handleChange = (value: number, id: number) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios
                    .patch(`http://localhost:3000/api/v1/orders/${id}`, { value })
                    .then((res) => {
                        if (res.status === 200) {
                            ...
                            {selectedOrderDetail?.map((item, i) => (
                                ...
                                >
                                    ...
                                    <div>
                                        <List>
                                            <ListItem>
                                                <ListItemText primary={"Thông tin: " + item.productName} />
                                            </ListItem>
                                            <ListItem>
                                                <ListItemText primary={"Giá: " + item.price} />
                                            </ListItem>
                                            <ListItem>
                                                <ListItemText primary={`Số lượng: ${item.quantity}`} />
                                            </ListItem>
